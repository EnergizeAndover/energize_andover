...
def header_parse(input_file):
    df = pd.read_csv(input_file, index_col='inputs')
    return df
...
def summarize(df, columns=None, start_time=None, end_time=None):
    ...
    if columns is None:
        raise ValueError('No column file was found')
    invalid_columns = set(columns.index) - set(df.columns)
    if invalid_columns:
        raise ValueError('the following column names were not found: {0}'.format(invalid_columns))
    subset = pd.DataFrame()
    nulltest = columns.isnull()
    cols = []
    for col in columns.index:
        if not nulltest.outputs[col]:
            subset[columns.outputs[col]] = df[col]
    #subset = df[cols]
